...
def pothole(request):
	...
	with open('{}\\'.format(settings.MEDIA_ROOT)+"feeds.csv", 'r') as log:
		...
		for row in read_logs:
			...
			data_Arr.insert(i,[timestamp,acc,latitude,longitude,ridescore])
			count+=1
			i+=1
		avg = total/count
		# print(data_Arr)
		for item in data_Arr:
			...
			item[0] = new_tstmp[0][:4] + "-" + new_tstmp[0][4:6] + "-" + new_tstmp[0][6:8] + " " + new_tstmp[1][:2] + ":" + new_tstmp[1][2:4] + ":" + new_tstmp[1][4:6]
			#print(item[0])
			acc_z = item[1]
			...
			if diff > 2:
				...
				if diff>=2 and diff<=4:
					...
					#print("High")
					intensity = "High"
				final_data.insert(k,[final_ts,item[2],item[3],item[4],intensity])
				k = k+1
			j+=1
		print(final_data)
...
def retrievefirebase():
	fire = firebase.FirebaseApplication('https://squid-3349a.firebaseio.com/', None)
	...
	locations = pd.DataFrame({'latitude': lat, 'longitude': lon, 'timestamp': timestampG})
	...
	def reduce(row):
		row['x'], row['y'], row['z'] = mean([row.x1, row.x2]), mean([row.y1, row.y2]), mean([row.z1, row.z2])
		return(row)
	...
	data.to_csv(os.path.join(settings.MEDIA_ROOT, 'data.csv'), sep=',', encoding='utf-8')
